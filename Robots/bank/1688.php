...
//        $real_request++;
        ...
        $contact_desc_1 = fetchOnce( $content,'<h3>联系方式<\/h3>.*?'.appendTags('div','class','m-footer') );
...
//        $start_time = time();
        ...
        if( !empty($contact) && count($contact)==3 )
        {
            ...
            $contact_name = str_cut_tail($contact_name,'和我联系');
            $contact_name = str_cut_tail($contact_name,'给我留言');
...
//1、false   不需要再次抓取
//2、-1      任何机器从来没有爬过，需要爬取
//3、>0      本机爬过一半意外中断，需要爬取，返回值为对应的行id
function check_position($posValue)
{
    ...
    if(!empty($infos))
    {
        foreach($infos as $pos_info)        //已完成或没有分配给本机的返回false不需要再次抓取数据
        {
